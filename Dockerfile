...
LABEL org.opencontainers.image.title="Simple Todo App" \
      ...
      org.opencontainers.image.authors="Robert Rompf <contact@rompf.dev>" \
...
# Use Gunicorn with production settings and proper timeouts
CMD ["gunicorn", "-w", "2", "-k", "gthread", "--threads", "4", "-b", "0.0.0.0:5000", "--timeout", "30", "--graceful-timeout", "30", "app:app"]
